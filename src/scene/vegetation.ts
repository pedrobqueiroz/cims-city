...
export function createTree(type: TreeType, palette: MaterialPalette): THREE.Group {
  ...
  // Trunk
  const trunkHeight = type === 'deciduous' ? 2.2 : 2.8;
  ...
  if (type === 'deciduous') {
    ...
    crown.position.y = trunkHeight + 1.6;
    crown.castShadow = true;
    crown.receiveShadow = true;
    crown.name = 'crown';
    tree.add(crown);
...
export function createBush(palette: MaterialPalette): THREE.Group {
  ...
  // Three small spheres clustered together
  ...
  for (let i = 0; i < positions.length; i++) {
    const sphere = new THREE.Mesh(
      new THREE.SphereGeometry(0.35 + i * 0.05, 6, 4),
      palette.context,
    );
